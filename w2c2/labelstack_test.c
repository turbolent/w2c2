...
testLabelStack(void) {
    ...
    CHECK(wasmLabelStackPush(&stack, 3, NULL, &label));
    CHECK(label.index == 1024 && label.typeStackLength == 3 && label.type == NULL);
    ...
    CHECK(wasmLabelStackPush(&stack, 0, NULL, &label));
    CHECK(stack.labels.labels[0].index == 0);
    CHECK(stack.labels.labels[0].typeStackLength == 0 && stack.labels.labels[0].type == NULL);
